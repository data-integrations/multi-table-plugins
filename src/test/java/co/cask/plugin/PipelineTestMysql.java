...
public class PipelineTestMysql extends HydratorTestBase {
    ...
    public static void setupTestClass() throws Exception {
        ...
        addPluginArtifact(NamespaceId.DEFAULT.artifact("mysql-jdbc", "1.0.0"),
                parentArtifact,
                Sets.newHashSet(mysqlPlugin), Class.forName("com.mysql.jdbc.Driver"));
    ...
    public void testMultiTableDump() throws Exception {
        ETLBatchConfig config;
        AppRequest<ETLBatchConfig> appRequest;
        ApplicationId appId;
        ApplicationManager appManager;
        long logicalStart;
        WorkflowManager workflowManager;
        DataSetManager<PartitionedFileSet> multi1Manager;
        DataSetManager<PartitionedFileSet> multi2Manager;
        DataSetManager<PartitionedFileSet> multi3Manager;
        DataSetManager<PartitionedFileSet> blacklist1;
        DataSetManager<PartitionedFileSet> blacklist2;
        long timePartition;
        PartitionKey partitionKey;
        Schema schema1;
        Schema schema2;
        Schema schema3;
        DataSetManager<Table> outputManager;
        List<StructuredRecord> outputRecords;
        Set<StructuredRecord> actual;
        ...
        appRequest = new AppRequest<>(APP_ARTIFACT, config);
        appId = NamespaceId.DEFAULT.app("multitable");
        appManager = deployApplication(appId, appRequest);

        logicalStart = System.currentTimeMillis();
        ...
        multi1Manager = getDataset("MULTI1");
        multi2Manager = getDataset("MULTI2");
        multi3Manager = getDataset("MULTI3");
        blacklist1 = getDataset("BLACKLIST1");
        blacklist2 = getDataset("BLACKLIST2");

        Assert.assertNull(blacklist1.get());
        Assert.assertNull(blacklist2.get());


        timePartition = TimeUnit.SECONDS.convert(logicalStart, TimeUnit.MILLISECONDS);
        partitionKey = PartitionKey.builder().addLongField("ingesttime", timePartition).build();
